...
class GeomCLIPBase(BaseModel):
    @classmethod
    def init_tokenizer(cls):
        if True:
            bert_name = 'allenai/scibert_scivocab_uncased'
            #bert_name = './pretrained/scibert'
        else:
    ...
    def init_graph_encoder(
        ...
        ckpt = torch.load('./pretrained/graphcl_80.pth', map_location=torch.device('cpu'))
    ...
    @classmethod
    def init_unimol_mol_encoder(cls, args):
        dictionary = Dictionary.load('./data/unimol_dict_mol.txt')
        dictionary.add_symbol("[MASK]", is_special=True)
        unimol_model = SimpleUniMolModel(args, dictionary)
        ckpt = torch.load('pretrained/mol_pre_no_h_220816.pt', map_location=torch.device('cpu'))['model']
    ...
    @classmethod
    def init_unimol_pro_encoder(cls, args):
        dictionary = Dictionary.load('./data/unimol_dict_pro.txt')
        dictionary.add_symbol("[MASK]", is_special=True)
        unimol_model = SimpleUniMolModel(args, dictionary)
        ckpt = torch.load('./pretrained/pocket_pre_220816.pt', map_location=torch.device('cpu'))['model']
        missing_keys, unexpected_keys = unimol_model.load_state_dict(ckpt, strict=False)
        # if len(missing_keys) or len(unexpected_keys):
        #     print(missing_keys)
        #     print(unexpected_keys)
